#!/usr/bin/env bash
# Import existing CFN-managed staging resources into Terraform state.
#
# Prerequisites:
#   - AWS CLI configured with credentials that have read access across
#     EC2, RDS, IAM, ELB, ECS, KMS, and Secrets Manager.
#   - Terraform >= 1.10 installed.
#   - S3 bucket routebox-tfstate-staging exists (with versioning + encryption).
#   - `acm_certificate_arn` in staging.tfvars replaced with the real ARN.
#   - Run from the environments/staging/ directory after `terraform init`.
#
# Usage:
#   export TF_VAR_rds_master_password="<real-password>"
#   bash import.sh
#
# The script is idempotent: re-running a line whose resource is already in
# state will print an error that can be safely ignored.
...
# Pass rds_master_password so TF config validates during import.
# The value is not written to state or applied to the resource.
...
echo "    nat eip alloc:    $NAT_EIP_ALLOC"
...
# Security group rule IDs for the separate aws_vpc_security_group_ingress_rule resources.
...
# Parameter group name is CFN-generated (not deterministic). Look it up
# from the instance's current association.
...
echo "    listeners: http=$HTTP_LISTENER_ARN  https=$HTTPS_LISTENER_ARN"
...
OPSCON_TG_ARN=$(aws elbv2 describe-target-groups --region "$REGION" \
  ...
  --query 'TargetGroups[0].TargetGroupArn' --output text)
echo "    target groups: shipments  routeopt  tracking  portal  opscon"

# Listener rules are looked up by priority (matches the CFN template).
...
echo "    listener rules: shipments  routeopt  tracking  opscon  portal"
...
# For each secret: resolve ARN and current version ID.
# Version ID is needed because aws_secretsmanager_secret_version imports as <arn>|<version_id>.
resolve_secret() {
  local secret_name="$1"
...
echo "    Next: terraform plan -var-file=staging.tfvars"
echo "    Expected result: no changes for imported resources."
echo "    Expected diffs to investigate before applying:"
echo "      - Tag ManagedBy: cloudformation -> terraform (cosmetic)"
echo "      - IAM inline policy drift (if live roles diverged from the CFN template)"
echo "      - ECS container insights setting (if toggled in console)"
